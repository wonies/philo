...
void	*routine(void *arg)
{
	...
	if (list->info->idx % 2 != 0)
	{
		...
		else if (list->next->use != 0 && list->use != 0)
		{
			if (list->status == 0)
			{
			pthread_mutex_lock(&(list->info->sleeping));
			pthread_mutex_lock(&(list->info->prints));
			printf("\t\033[0;31m %d philosopher is sleeping\n\033[0m", list->info->idx);
			pthread_mutex_unlock(&(list->info->prints));
			...
			else 
			{
				...
				pthread_mutex_lock(&(list->info->prints));
				printf("\t\033[0;32m %d philosopher is thinking\n\033[0m", list->info->idx);
	...
	else if (list->info->idx % 2 == 0)
	{
		...
		else if (list->prev->use != 0 && list->use != 0)
		{
			if (list->status == 0)
			{
				pthread_mutex_lock(&(list->info->sleeping));
